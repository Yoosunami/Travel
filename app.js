const storageKey = "sumi-travel-desk-v2";
const previousStorageKey = "sumi-travel-desk-v1";
...
function loadState() {
  ...
  try {
    const oldState = JSON.parse(localStorage.getItem(previousStorageKey));
    if (oldState) return normalizeState(migrateOldState(oldState));
  } catch {
...
function normalizeState(value) {
  return {
    ...defaultState,
    ...value,
    travelers: Math.max(1, Number(value.travelers) || defaultState.travelers),
    budgetAmount: Math.max(0, Number(value.budgetAmount) || defaultState.budgetAmount),
    days: Array.isArray(value.days) ? value.days : defaultState.days,
...
function renderDays() {
  ...
  document.querySelector("#chatPromptLink").href = chatLink(
    "請幫我重新優化這趟旅行的每日行程，保留穿搭拍照、美食、交通效率，並列出風險與備案。",
  );
}
...
function bindEvents() {
  ["tripName", "travelers", "startDate"].forEach((id) => {
    document.querySelector(`#${id}`).addEventListener("input", (event) => {
      state[id] = id === "travelers" ? Math.max(1, Number(event.target.value) || 1) : event.target.value;
      saveState();
      renderAll();
    });
  });
  ...
  ["budgetMode", "budgetAmount"].forEach((id) => {
    document.querySelector(`#${id}`).addEventListener("input", (event) => {
      state[id] = id === "budgetAmount" ? Math.max(0, Number(event.target.value) || 0) : event.target.value;
      saveState();
      renderAll();
    });
  });
